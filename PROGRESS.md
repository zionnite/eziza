...
### Monetisation — Phases 2+ (all since built — this section is historical)
- [x] Markup on external carrier quotes — was blocked on Shipbubble integration at the time this was written; built 2026-08-11, see the Monetisation section further down
- [x] Admin earnings dashboard — `eziza-admin` exists now (Phase 2, 2026-07-10) with a dedicated Earnings page
- [x] Tenant billing ledger — `eziza-admin`'s "Tenant Billing" page reports commission owed per tenant from `earnings_ledger`, though real invoicing/collection still doesn't exist (still just a reporting view, as predicted here)
...
### Monetisation
- [x] Commission deduction in `pay_and_accept_delivery_bid` RPC — done via `credit_delivery_earnings()`, see Phase 1 above
- [x] Markup on external carrier quotes — done 2026-08-11, see below
- [x] `delivery_fee_breakdown` jsonb column on `deliveries` — done, see Phase 1 above
- [x] Admin earnings dashboard — `eziza-admin` Earnings page exists
- [x] **Uncollected-commission gap: a partner's delivery fulfilled by an Eziza rider/company — FIXED + live-verified 2026-08-11.** Confirmed by tracing the actual money flow (user asked for a full audit after sensing something was off between Eziza Direct, Eziza Partners, and Shipbubble): when a tenant like ZeeFashion routes a delivery to an *Eziza rider/company* (not Shipbubble), the tenant collected the full delivery fee from its own customer, but Eziza's own `accept-bid` edge function never touched money at all — the rider/company still got paid in full once confirmed, with nothing ever collected from the tenant. Documented as a *known, previously-accepted* trade-off in `20260730000000_external_carriers_for_tenants.sql` (External Carrier bookings were deliberately required to prepay *because* this gap already existed), but decided 2026-08-11 to close it rather than leave it accepted, going with the option that reuses existing infrastructure: **option (2)** from the original TODO — draw from the tenant's existing prepaid wallet at accept time, same mechanism External Carrier already uses.
  ...
  - **Not yet touched, deliberately out of scope for this fix**: Shipbubble's own no-markup pass-through (options 2 and 4 from the original money-flow map) — that's missed margin, not an active loss, and a separate decision. (Since built — see below.)

- [x] **Shipbubble commission — BUILT + LIVE-VERIFIED 2026-08-11.** External Carrier bookings were pure pass-through in both directions (Eziza Direct and tenants alike paid exactly the carrier's raw quote, no markup) — the missed-margin gap flagged as out of scope in the fix above. Takes the same `settings.platform_fee_pct` commission already applied to internal-rider deliveries rather than a second rate to separately configure and reason about — one unified "Eziza platform commission" concept across both fulfillment channels.
  - New shared `_shared/commission.ts` (`getPlatformFeePct`/`applyCommission`), used by both `shipbubble-fetch-rates` (Direct customers) and `tenant-shipbubble-rates` (tenants) — marks up the raw carrier total at the point the quote is first stored (migration `20260811030000_shipbubble_commission.sql`), so everything downstream (the picker UI and the payment RPCs) automatically uses the marked-up number with no other code changes needed.
  - `external_carrier_quotes`/`external_carrier_bookings` gain `carrier_cost`/`commission_amount` columns so Eziza's actual Shipbubble cost obligation stays distinguishable from what was collected — same reasoning `earnings_ledger` already applies to internal-rider commission, just as two columns here since a Shipbubble booking has no rider/company row to hang a ledger entry off. Carried forward from quote to booking at commit time by `finalize_book_external_carrier[_for_tenant]`, same "carry it forward" pattern already used for pickup/delivery ETA.
  - **Redeploy note**: `tenant-shipbubble-rates` needed `--no-verify-jwt` on this redeploy — caught immediately via the standard garbage-JWT check before it could actually break anything in production (same recurring gotcha as `logistics-gateway` the same day — see `[[feedback_verify_jwt_redeploy]]` if that memory exists, or the note in this file's `accept-bid` entry).
  - **Live-verified** the RPC/wallet path with throwaway tenant + delivery + quote data (`carrier_cost=1000`, `commission=100` at the current 10% rate): tenant correctly debited the full marked-up ₦1,100, and `carrier_cost`/`commission_amount` correctly carried forward onto the booking. The rate-fetch arithmetic itself (simple, low-risk) was verified by direct review rather than a full live carrier round-trip, to avoid spending real Shipbubble sandbox credits on this change alone.
