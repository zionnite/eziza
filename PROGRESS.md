...
- [ ] **Not yet done**: register `https://nvwpsccleewgirlwokys.supabase.co/functions/v1/paystack-webhook` as the webhook URL in the Paystack dashboard for the account owning `PAYSTACK_SECRET_KEY` — user confirmed in-progress. Two ₦10,000 top-ups from testing before registration were found via the new admin-only `paystack-list-recent` function (lists recent Paystack transactions) but deliberately **not credited yet** — pending the user confirming those payments are actually theirs, since crediting a wallet is real money movement.
- [ ] The new embedded-WebView checkout page has not yet been confirmed working end-to-end by the user (the 3 iterations above were each tested live, but the final WebView version was just built and deployed)
