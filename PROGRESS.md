...
### External Carriers / Shipbubble — BUILT 2026-07-29, pending Shipbubble account approval for live E2E
Customer-facing alternative to rider/company bidding: get instant courier quotes (DHL, GIG, etc.) and book directly, alongside the existing "Offers Received" section. Shipbubble account is signed up but **not yet approved** — full live testing (real address validation, real rates, real booking, real webhooks) is blocked on that, same situation Sendchamp's account-verification gate put the SMS integration in. Everything that could be verified without a live account was verified.
...
- [x] 5 new edge functions, all deployed and auth-smoke-tested (correctly return 503 "Shipbubble is not configured yet" rather than crashing, since `SHIPBUBBLE_API_KEY` isn't set pending approval):
  ...
  - `shipbubble-webhook` — verifies `x-ship-signature` (HMAC-**SHA512**, not the SHA256 used elsewhere in this project — Shipbubble's own scheme), maps their status events to booking status + reflects `completed`/`cancelled` onto `deliveries.status`. Exact webhook payload field names are unconfirmed against a real event (account not approved) — written defensively with fallback field-name candidates, full raw payload always preserved in `status_history` regardless of whether a guess was right.
...
- [x] `flutter analyze` clean across the whole `lib/` tree.
- [ ] **Not live-tested end-to-end** — blocked on Shipbubble account approval. Once approved: verify real address validation, real rate quotes, real booking + compensating-cancel path, and — importantly — confirm the *real* webhook payload field names match what `shipbubble-webhook` guessed (categories/dimensions response field names too, used defensively in the Flutter picker).
...
- [ ] **Terminal Africa (T-Ship API)** — still flagged as an alternative worth evaluating (same aggregator model, additive architecture above should transfer with a different payload adapter). Not evaluated. Sendbox skews toward social-commerce sellers; Kwik is a single on-demand courier, not an aggregator — neither is a real alternative to Shipbubble's slot.
- [ ] **Admin panel — manage carriers + manual rate cards** — explicitly out of scope for this build, not requested.

### External Carriers — extended to tenants (ZeeFashion + eziza-partners) — BUILT 2026-07-30, same Shipbubble-approval caveat as above
...
| Sendchamp SMS | `confirm-delivery-otp`'s `sendSms()` switched from Termii to Sendchamp (`SENDCHAMP_API_KEY` secret set). **API accepts every request (`status: success`) but the SMS never actually arrives** — confirmed via live test to a real phone, dashboard shows the message flips to "failed" after initially showing success. Tried both `sender_name` variants (`Sendchamp`, `ChampOTP`) and both `route` values (`dnd`, `non_dnd`) — identical failure every time, and account balance confirmed sufficient. Points to an account-verification/KYC gate on Sendchamp's side rather than anything in our code. Reference `business_id` for their support: `93467d23-0b9a-4009-9cdc-d54edcc7c080`. Code left on `dnd` route (the correct one for real use) + `ChampOTP` sender, with temporary debug instrumentation still in `sendSms()`/the `send` action response (`_sms_debug_TEMP`) to keep diagnosing — strip that once real delivery is confirmed working. |
