...
- [ ] **Uncollected-commission gap: a partner's delivery fulfilled by an Eziza rider/company — deferred, revisit later (2026-08-11)**. Confirmed by tracing the actual money flow (user asked for a full audit after sensing something was off between Eziza Direct, Eziza Partners, and Shipbubble): when a tenant like ZeeFashion routes a delivery to an *Eziza rider/company* (not Shipbubble), the tenant collects the full delivery fee from its own customer into its own wallet — `zeefashion: finalize_accept_eziza_bid()` — but Eziza's own `accept-bid` edge function never touches money at all, and `credit_delivery_earnings()` (the trigger that pays the rider on `deliveries.status='confirmed'`) fires exactly the same as it does for an Eziza Direct customer, with zero awareness any of this happened. So the rider/company gets paid in full with nothing ever collected from the tenant. This is a *known, previously-accepted* trade-off, not new — see the comment in `20260730000000_external_carriers_for_tenants.sql` explaining that External Carrier bookings were deliberately required to prepay *because* this gap already existed and was accepted as the smaller risk (a soft commission owed, not a hard third-party payable). `/api/admin/billing` (eziza-admin's "Tenant Billing" page) already reports the commission owed per tenant from `earnings_ledger`, but explicitly does nothing to invoice or collect it. Options sketched out (not decided): (1) turn Tenant Billing into a real periodic invoice + collect like any B2B platform fee, (2) require internal-rider deliveries to draw from the tenant's existing prepaid wallet at accept time, same as External Carrier already does, (3) take Eziza's cut via a Paystack split at the tenant's own checkout instead of invoicing after the fact.
