...
- [ ] **Auto-recharge for tenant wallets — scoped 2026-08-11, not started, optional fast-follow to the fix above.** Motivation: a high-volume tenant can't sensibly guess a lump sum to prepay, and now that internal-rider deliveries also debit the wallet at accept time (not just External Carrier bookings), that balance drains faster and more often. Decided against invoicing/postpaid as the general answer — that just re-opens the same "Eziza fronts money with no guaranteed collection" exposure this fix closes, and should stay a manual, case-by-case credit decision for a specifically-trusted tenant if it ever happens at all, not a parallel billing mode.
  - **Shape of the fix, for whenever this is picked up**: an opt-in toggle in `eziza-partners`' own dashboard (default OFF — every tenant keeps today's manual top-up behavior unless they turn it on) — "When my balance drops below ₦X, automatically charge my card ₦Y." Same UX pattern Twilio/AWS use for exactly this problem.
  - **Mechanism**: Paystack authorization reuse (`authorization_code`, returned on the *first* successful `charge.success` webhook event for that tenant) lets a later charge run without the tenant re-entering card details — `POST /transaction/charge_authorization` server-side, same trust boundary as `tenant-paystack-initialize`/`paystack-webhook` already established. Needs: `tenants.auto_recharge_enabled`/`auto_recharge_threshold`/`auto_recharge_amount` + a stored authorization code (from the tenant's *own* successful top-up, never captured any other way), and a check — cheapest is probably right inside `accept_bid_for_tenant`/`finalize_book_external_carrier_for_tenant` after a successful debit (if `wallet_balance < threshold` and auto-recharge is on, fire the recharge) rather than a separate polling job.
